...
<template>
  <div class="list-comp">
    <div class="text-3xl text-center h-16 leading-loose">同学名单</div>
    <div class="tool-bar">
...
//方法不需要响应式storeTorefs不会去处理方法，需要userStore结构
...
const editUser = index => {
  isEdit.value = true;
  curIdx.value = index;
  const item = useUserStore.userList[index];
  // item.age = 22;
...
const checkList = (code) =>{
  for (let index=0;index<useUserStore.userList.length;index++){
  const element = useUserStore.userList[index];
...
const addNum2name=(name)=>{
  //姓名后追加最新编号
  let total=0;
  useUserStore.userList.map(item =>{
...
const subMitFn=()=>{
  ...
  if (isEdit.value){
  useUserStore.userList.splice(curIdx.value,1);
  tempName = name.value;
  }
  ...
  useUserStore.userList.unshift({
    id:studyNum.value,
    userName:tempName,
    age:year.value
  })
  console.log(useUserStore.userList);
  showFlag.value=false;
  }

  const getYourName = id =>{
    const student=useUserStore.userList.find(item =>item.id ===id);
